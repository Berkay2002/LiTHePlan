...
export function CourseFAQSchema({ course }: CourseFAQSchemaProps) {
  ...
  const examinationMethods =
    Array.isArray(course.examination) && course.examination.length > 0
      ? course.examination
          .map((exam) => EXAMINATION_MAPPING[exam] || exam)
          .join(", ")
      : "various assessment methods";

  // Build program list
  const programCount = Array.isArray(course.programs)
    ? course.programs.length
    : 0;
  const programList = Array.isArray(course.programs)
  ...
  const termsText = Array.isArray(course.term)
    ? course.term.join(", ")
    : course.term;
  const periodsText = Array.isArray(course.period)
    ? course.period.join(", ")
    : course.period;
