...
interface ConflictResolutionModalProps {
  ...
  onClose: () => void;
  newCourse: Course;
  conflictingCourses: { conflictingCourse: Course; conflictingCourseId: string }[];
...
export function ConflictResolutionModal({
  ...
  const [selectedExistingCourse, setSelectedExistingCourse] = useState<Course | null>(null);
      ...
      <DialogContent className="max-w-3xl max-h-[85vh] overflow-y-auto bg-gradient-to-br from-card to-card/95 border-2 border-picton-blue/20 shadow-2xl">
        <DialogHeader className="text-center pb-6">
          ...
          <DialogDescription className="text-base text-muted-foreground mt-2">
            These courses cannot be taken together. Please choose which one you&apos;d like to keep in your profile.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-6">
              ...
              <div className="flex items-start gap-4">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-2">
                    <h3 className="text-lg font-semibold text-picton-blue">New Course</h3>
                    <span className="text-sm text-muted-foreground">(Add this one)</span>
                  </div>
                  <h4 className="font-semibold text-foreground mb-3 leading-tight">
                    {newCourse.name}
                  </h4>
                  <div className="mb-3">
                    <Badge variant="secondary" className="bg-picton-blue/15 text-picton-blue border-picton-blue/30 font-medium">
                      {newCourse.id}
                    </Badge>
                  </div>
                  {newCourse.notes && (
              ...
              <div className="flex items-start gap-4 mb-4">
                <div className="flex-1">
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <span className="text-sm text-muted-foreground">(Keep existing)</span>
                  </div>
                  <p className="text-sm text-muted-foreground">
                    {conflictingCourses.length === 1 ? 'This course is' : 'These courses are'} already in your profile
                  </p>
                </div>
                
                {/* Selection indicator for single course */}
                {conflictingCourses.length === 1 && selectedChoice === 'existing' && (
              ...
              <div className="space-y-3">
                {conflictingCourses.map(({ conflictingCourse }) => (
                  <div
                    ...
                    className={`relative border rounded-lg p-4 cursor-pointer transition-all duration-200 ${
                      conflictingCourses.length === 1
                        ...
                        : selectedChoice === 'existing' && selectedExistingCourse?.id === conflictingCourse.id
                  ...
                  >
                    ...
                    {conflictingCourses.length > 1 && selectedChoice === 'existing' && selectedExistingCourse?.id === conflictingCourse.id && (
          ...
          <div className="flex justify-between items-center pt-6 border-t border-border/50">
            <p className="text-sm text-muted-foreground">
              {selectedChoice ? (
                selectedChoice === 'new' ? 'New course will be added' : 'Current course will be kept'
              ) : (
            ...
            <div className="flex gap-3">
              ...
              <Button
                ...
                disabled={!selectedChoice || (selectedChoice === 'existing' && !selectedExistingCourse)}
