  ...
  courses.forEach(course => {
    ...
    if (Array.isArray(course.programs)) {
      course.programs.forEach(program => {
        ...
        stats.coursesPerProgram[program] = (stats.coursesPerProgram[program] || 0) + 1;
      });
    }
  });
  
  // Calculate average credits
  stats.averageCredits = courses.reduce((sum, course) => sum + course.credits, 0) / courses.length;
...
function displayStatistics(stats) {
  ...
  console.log(`   Courses with notes: ${stats.withNotes} (${notesPercentage}%)`);
  console.log(`   Courses without notes: ${stats.withoutNotes} (${(100 - notesPercentage).toFixed(1)}%)`);
  
  console.log(`\n🎯 PROGRAM COVERAGE:`);
  console.log(`   Total unique programs: ${stats.uniqueProgramsArray.length}`);
  console.log(`   Average credits per course: ${stats.averageCredits.toFixed(1)}hp`);
  ...
  console.log(`Advanced Level: ${stats.byLevel['avancerad nivå'] || 0} courses`);
  console.log(`Basic Level: ${stats.byLevel['grundnivå'] || 0} courses`);
  console.log(`Campus Distribution: Linköping (${stats.byCampus['Linköping'] || 0}), Norrköping (${stats.byCampus['Norrköping'] || 0}), Distance (${stats.byCampus['Distans'] || 0})`);
  console.log(`Credit Distribution: ${Object.entries(stats.byCredits).map(([credits, count]) => `${credits} (${count})`).join(', ')}`);
  console.log(`Programs Covered: ${stats.uniqueProgramsArray.length} different programs`);
  console.log(`Courses with Restrictions: ${stats.withNotes} (${((stats.withNotes / stats.total) * 100).toFixed(1)}%)`);
}
