...
async function importCourses() {
  ...
  try {
    ...
    const coursesPath = path.join(process.cwd(), 'data', 'new-real-courses.json');
    ...
    for (let i = 0; i < coursesData.length; i += batchSize) {
      ...
      console.log(`Importing batch ${Math.floor(i/batchSize) + 1}/${Math.ceil(coursesData.length/batchSize)} (${batch.length} courses)`);
      
      try {
        const { data, error } = await supabase
          .from('courses')
          .upsert(batch.map(course => ({
            id: course.id,
            name: course.name,
            credits: parseFloat(course.credits), // Handle decimal credits like 7.5
            level: course.level,
            term: course.term || [],
