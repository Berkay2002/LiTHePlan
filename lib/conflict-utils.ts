...
export function detectCourseConflicts(courses: Course[], targetCourse: Course, currentPeriod: 1 | 2): ConflictInfo | null {
  ...
  if (is50Percent && Array.isArray(targetCourse.block)) {
    targetBlocks = [parseInt(targetCourse.block[currentPeriod - 1])];
  } else {
    targetBlocks = Array.isArray(targetCourse.block) 
      ? targetCourse.block.map(b => parseInt(b)) 
      : [parseInt(targetCourse.block)];
  }
  ...
  const periodCourses = courses.filter(course => {
    ...
    const courseInPeriod = (Array.isArray(course.period) ? course.period.includes(currentPeriod.toString()) : course.period === currentPeriod.toString()) || course.pace === '50%';
  ...
  for (const otherCourse of periodCourses) {
    ...
    if (otherIs50Percent && Array.isArray(otherCourse.block)) {
      otherBlocks = [parseInt(otherCourse.block[currentPeriod - 1])];
    } else {
      otherBlocks = Array.isArray(otherCourse.block) 
        ? otherCourse.block.map(b => parseInt(b)) 
        : [parseInt(otherCourse.block)];
    }

    // Check for block overlaps
    const overlappingBlocks = targetBlocks.filter(block => otherBlocks.includes(block));
  ...
  // Remove duplicates
  conflicts.conflictDetails.blockConflicts = [...new Set(conflicts.conflictDetails.blockConflicts)];
  conflicts.conflictDetails.conflictingCourses = [...new Set(conflicts.conflictDetails.conflictingCourses)];
...
export function getConflictBorderClass(hasConflict: boolean): string {
  return hasConflict 
    ? 'border-red-500 border-2 shadow-red-200 shadow-md' 
    : '';
}

export function getAllPeriodConflicts(courses: Course[], period: 1 | 2): Map<string, ConflictInfo> {
  const conflictMap = new Map<string, ConflictInfo>();
  
  // Filter courses for this period
  const periodCourses = courses.filter(course => {
    return (Array.isArray(course.period) ? course.period.includes(period.toString()) : course.period === period.toString()) || course.pace === '50%';
  });
