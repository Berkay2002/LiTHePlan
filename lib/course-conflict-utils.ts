...
export function extractConflictingCourses(
  ...
  const englishPattern =
    /The course may not be included in a degree together with:\s*([A-Z0-9, ]+)/i;
  const swedishPattern =
    /Kursen får ej ingå i examen tillsammans med:\s*([A-Z0-9, ]+)/i;
...
export function findCourseConflicts(
  ...
  MASTER_PROGRAM_TERMS.forEach((term) => {
    ...
    termCourses.forEach((existingCourse) => {
      ...
      const existingCourseConflicts = extractConflictingCourses(
  ...
  return conflicts.filter(
    (conflict, index, arr) =>
      arr.findIndex(
        (c) => c.conflictingCourseId === conflict.conflictingCourseId
      ) === index
  );
}
...
export function doCoursesConflict(courseA: Course, courseB: Course): boolean {
  ...
  return (
    courseAConflicts.includes(courseB.id) ||
    courseBConflicts.includes(courseA.id)
  );
}
